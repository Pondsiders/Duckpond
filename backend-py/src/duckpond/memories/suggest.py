...
Results accumulate in Redis for the Loom to inject as "Intro suggests".

This is the thick-client implementation of Intro's memorables extraction,
...
async def _store_memorables(session_id: str, memorables: list[str]) -> None:
    ...
    try:
        ...
        logfire.info("Memorables stored in Redis", session_id=session_id[:8], count=len(memorables))
    finally:
...
async def suggest(user_content: str, assistant_content: str, session_id: str) -> None:
    ...
    Results accumulate in Redis at intro:memorables:{session_id} for
    the Loom to inject on subsequent turns.
    ...
    """
    with logfire.span("memories.suggest", session_id=session_id[:8] if session_id else "none"):
